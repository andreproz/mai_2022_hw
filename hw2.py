...
with open("pilot_path.json") as pilot_path_file:
    pilot_path = json.load(pilot_path_file)
...
for pilot in pilot_path:
    pilot_mission_dict[pilot] = len(pilot_path[pilot]['missions'])
...
drone_models_list = set()

for pilot in pilot_path:
    for mission in pilot_path[pilot]["missions"]:
        drone_models_list.add(mission["drone"])
        
# вывод результата (допишите код)
print(f'Полеты совершались на дронах следующих моделей: {", ".join(drone_models_list)}')
print()
...
drone_missions_list = list()

for pilot in pilot_path:
    for mission in pilot_path[pilot]["missions"]:
        drone_missions_list.append(mission["drone"])
        
# вывод результата (допишите код)
for drone_model in drone_models_list:
    print(f"Дрон {drone_model} отлетал {drone_missions_list.count(drone_model)} миссий")
...
class UAV:
	...
	@missions.setter
	def missions(self, added_mission):
	    self._missions.append(added_mission)
...
for drone_model in drone_models_list:
    new_drone_model = MultirotorUAV(
        weight = drone_catalog[drone_model]["weight"],
        model = drone_model,
        brand = drone_catalog[drone_model]["brand"]
    )
    missions = []
    for pilot in pilot_path:
        for mission in pilot_path[pilot]["missions"]:
            if mission["drone"] == drone_model:
                new_drone_model.missions.append(mission["mission"])
    drones_list.append(new_drone_model)
...
for drone_model in drones_list:
    if drone_model.get_model() == fixed_drone_names[str(user_input).lower()]:
        print(f"Информация о дроне {drone_model.get_model()}:{drone_model.get_info()}.")